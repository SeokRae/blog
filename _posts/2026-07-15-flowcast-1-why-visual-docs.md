...
필요한 그림이 하나 생길 때마다 그 관점을 다루는 방법을 별도의 스킬로 만들었다. 지금의 `flowcast`는 입력을 다이어그램 단위로 나누고, 무엇을 보여줘야 하는지에 따라 이 스킬들을 선택해 연결한다. 하나의 자료에서 여러 그림이 필요하면 각각의 스킬이 맡을 단위로 나눠 동시에 처리한다.
